...
int10_set_vm:
  ...
  emu_putchar_al
  pop   ax
  call  puts_hex_al  ; the video mode number printed to debug output.  For example, full printed line could be: VID03
  push  ax
  mov   al, 0x0a
  emu_putchar_al
  mov   al, 0x0d
...
int10_set_vm_0d:     ; EGA 320x200 16 COLOR
int10_set_vm_0e:     ; EGA 640x200 16 color
int10_set_vm_0f:     ; EGA 640x350 monochrome 2 color
int10_set_vm_10:     ; EGA 640x350 4 color (for 64K) or 16 color (for 128K)
int10_set_vm_12:     ; VGA 640x480 16 color
emu_video_handler
jmp   int10_set_vm_upd
...
cur_not_visible:
; BIOS_CGA.ASM: mov	byte [cursor_visible-bios_data], 0	; Hide cursor
  mov   cx, ax
  jmp   cur_vischk_done
...
cur_visible:
  ...
  or    ax, 0x6000
  mov   cx, ax
; BIOS_CGA.ASM: mov	byte [cursor_visible-bios_data], 1	; Show cursor
...
int10_write_char_attrib_repeat:
  ...
  ; handle video mode 0x0d
  ;cmp    byte [vid_mode-bios_data], 0x0d
  ;je     int10_write_char_attrib_mode0d
...
int10_write_char_attrib_mode0d:
  call   put_mode0d_char
  jmp    int10_write_char_attrib_next  
...
int10_vid_combination:
  ...
  ;mov  bl, 0x0b   ;MCGA with monochrome display
  mov  bl, 0x0C  ; MCGA with colour display, 2013-06-03  //did I mean 2023?
  ;mov   bl, 0x02 ; CGA with color display
  ;mov bl, 0x08

  ;mov   bh, 0  ; no inactive display
  mov   bh, 0x01  ; MDA/Hercules with monochrome display
  mov   al, 0x1a
...
int10_get_state_info:
  ...
  ;mov  byte [es:di+0x25], 0x07    ; active display combination code (0x07=MCGA)
  ;mov  byte [es:di+0x25], 0x08    ; active display combination code (0x08=VGA with analog color display)
  ;mov  byte [es:di+0x26], 0x00    ; inactive display combination code
  mov  byte [es:di+0x26], 0x01    ; inactive display combination code (0x01=MDA with monochrome display)
  mov  word [es:di+0x27], 0x0010  ; Number of displayed colours
...
int10_get_state_info_pages_done:
  ...
  ; below added by Eric 2023-06-06
  mov byte [es:di+0x31], 0x00     ; video RAM available 0=64K, 1=128K, 2=192K, 3=256K
  mov byte [es:di+0x32], 0x00     ; save area status (related to custom font overriding, I think)
...
int15_getextmem:
	...
	db 0Fh, 04h		; call into XMS interface   // here writing "db XXh, XXh" effectively writes the machine language instruction XXXX (two bytes) into the program at this location.  In this case, it runs the custom 0f04 instruction, a BIOS helper function for xms in 8086tiny.
...
int16_00_wait:
  ...
  ;jz int16_00_wait
  jnz int16_00_done  ;if key ready, done
  sti
  hlt ; halt cpu until hardware interrupt (i.e. keystroke)
  jmp int16_00_wait  ; after wakeup from halt, check key buffer again
int16_00_done:
  iret
...
int17:
  ...
  ; print contents of AH has hex value
  pop ax
  push ax
  mov al, ah
  call  puts_hex_al  
  ...
  ; print contents of AL has hex value
  pop ax
  push ax
  call  puts_hex_al
...
clear_window_gfx320:
  ...
  ; the following 2 lines get the row number multiplied by 80, which is character offset on a 40x25 display
  mov  al, 80 ; bytes per row
  mul  byte ch
  ; the following 2 lines multiply the character offset obtained above by 4, since there are 4 pixels per byte, resulting in the pixel offset of the upper left pixel to be cleared.  Shouldn't we be dividing by 4 instead of multiplying by 4, since we are calculating the memory address of the byte that has the top 4 pixels of the upper left corner of the window?
  shl  ax, 1
  shl  ax, 1
  ; the above 4 lines of code effectively multiply the row number by 320
  ; the following 4 lines add the column number to ax twice (shouldn't it be 4 times, since there are 4 pixels per byte?)
  add  al, cl
  adc  ah, 0
  add  al, cl
  adc  ah, 0
  ; the following 1 line of code sets di to the final calculated pixel offset
  mov  di, ax

  ; bl = number of rows in the window to clear
  ; the following 2 lines calculate the difference between the upper row number and the bottom row number of the window to clear, and store it in bl
  mov  bl, dh
  sub  bl, ch
  inc  bl; this line added by Eric 2023-07-23 to fix bug that caused odd lines to be wiped clear on the whole screen.  This finally fixes the King's Quest 1 start screen bug!
  ; the following 2 lines multiply the number of rows calculated above by 4 (shouldn't we multiply by 8 since there are 8 pixel rows per text character?)
  shl  bl, 1
  shl  bl, 1
  ; cx = number of words to clear per row
  push ax
  mov  ax, 0
  mov  al, dl
  sub  al, cl
...
put_mode0d_char:
  ...
  jmp  put_mode0d_char_vidoffset

put_mode0d_char_high:
  ; Characters 128 .. 255 get their address from interrupt vector 1F
  and  al, 0x7F
  mov  ah, 0
  shl  ax, 1
...
put_mode0d_char_vidoffset:
  ...
  shl  bx, 1
  add  ax, bx
  mov  di, ax ; below crashing probably comes from here.  Crashes probably because di=d800, this is too high
  ...
  mov  cl, 8
put_mode0d_char_loop:
  mov  bl, byte [ds:bp]

  mov  ch, 0x80
put_mode0d_char_line_loop:
  test bl, ch
  jz   put_mode0d_char_clear
  mov  al, bh
  jmp  put_mode0d_char_pixel
put_mode0d_char_clear:
  mov  al, 0x00
put_mode0d_char_pixel:
  ...
  shr  ch, 1
  jnz  put_mode0d_char_line_loop

  inc  bp
  add  di, 312
  dec  cl
  jnz  put_mode0d_char_loop

put_mode0d_char_done:
  pop  bp
  pop  di
  pop  es
  pop  ds
...
com3addr         dw  0x03E8
com4addr         dw  0x02E8
lpt1addr         dw  0x0378              ; 40:08h  uhhh.. Isn't 0x0378 LPT2?
lpt2addr         dw  0
lpt3addr         dw  0
lpt4addr         dw  0                   ; 40:0e
...
;equip            dw  0b0101010001100101  ; 40:10h  ; setting bit 14 to 1 should mean 1 paralell port exists?
 ;                      5432109876543210
