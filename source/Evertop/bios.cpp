...
uint8_t floppyBuf[512];  // for floppy disk reading.  Waste this 512 bytes permanently in order to avoid using malloc()
...
void BIOS::init(Machine * machine)
{
  ...
  m_MC146818  = m_machine->getMC146818();
	...
	// copy bios into RAM
  // this kind of wastes 16K of precious ESP32 RAM.  Could add some RAM access logic to redirect reads from BIOS area to PROGMEM
  // of course, then software wouldn't be able to change the BIOS code, which quite a bit of software does do.
  printf("sizeof(biosrom) = %d\n", sizeof(biosrom));
...
void BIOS::helpersEntry()
{
  switch (i8086::AH()) {
    ...
    case 0x06:
      printf("pointingDeviceInterface()\n");
...
void BIOS::getKeyFromBufferPublic()
{
  getKeyFromBuffer();
}
...
{
  //printf("inside BIOS::getKeyFromKeyboard()\n");
  uint8_t * flags1 = m_memory + BIOS_DATAAREA_ADDR + BIOS_KBDSHIFTFLAGS1;
  uint8_t * flags2 = m_memory + BIOS_DATAAREA_ADDR + BIOS_KBDSHIFTFLAGS2;
...
bool BIOS::storeKeyInKbdBufferPublic(uint16_t syscode)
{
  return storeKeyInKbdBuffer(syscode);
  /*
    // check space in BIOS keyboard buffer
    auto head = (uint16_t*)(m_memory + BIOS_DATAAREA_ADDR + BIOS_KBDBUFHEAD);
    auto tail = (uint16_t*)(m_memory + BIOS_DATAAREA_ADDR + BIOS_KBDBUFTAIL);
...
{
  //printf("inside BIOS::getKeyFromBuffer()\n");
  ...
  {
    printf("calling m_keyboard->setLEDs()\n");
...
{
  //printf("inside pointingDeviceInterface()\n");
  //delay(100);
...
void BIOS::syncTicksWithRTC()
{
  ...
  *(uint32_t*)(m_memory + BIOS_DATAAREA_ADDR + BIOS_SYSTICKS) = (uint32_t) (pitTicks / 65536);
  printf("t=%d\n", *(uint32_t*)(m_memory + BIOS_DATAAREA_ADDR + BIOS_SYSTICKS));
}
...
void BIOS::diskHandler_floppy()
{
  ...
  switch (service) {
    ...
    case 0x05:
    {
      ...
      //uint8_t * buf = (uint8_t*) malloc(512);
      //memset(floppyBuf, fillByte, 512 * sizeof(uint8_t));
      for (int i=0; i < 512; ++i)    // Set the 512 elements (0-511) in floppyBuf
      {
        floppyBuf[i] = fillByte;        // to the value fillByte.
      }
      for (int i = 0; i < sectsCountToFormat; ++i) {
        ...
        fwrite(floppyBuf, 1, 512, m_machine->disk(drive));
...
void BIOS::diskHandler_HD()
{
  ...
  switch (service) {
    ...
    // The following is broken for saving and loading video memory because the emulator keeps
    // video memory in a separate memory addressing space than system RAM.
    // Therefor need to check if the address is between 0xA0000 and 0xBFFFF (inclusive) and
    // read/write using the appropriate video memory address as needed.
    // Also need to somehow trigger screen refresh after writing to video RAM
    ...
    case 0x04:
    {
      ...
      if (service != 0x04)
      {
        if (service == 0x02)
        {
          ...
          sects = fread(m_memory + dest, 1, count, m_machine->disk(drive));
          //printf("read took %dms\n", millis() - starttime);
          //printf("READ disk: sects = %d = fread(0x%02x, 1, %d, %s);\n", sects, dest, count, m_machine->diskFilename(drive));
          if (dest >= 0xA0000 && dest <= 0xBFFFF)
          {
            printf("READ disk to VRAM: sects = %d = fread(0x%02x, 1, %d, %s);\n", sects, dest, count, m_machine->diskFilename(drive));
            for (int n = 0; n < count; n++)
            {
              m_machine->writeVideoMemory8(m_machine, dest + n, *(m_memory + dest + n));
              
//              // Dump the written video RAM to serial port output as pixels, works for 1-bit graphics mode debugging
//              char buffer[16], len= 0;
//              for (int mask = 0x8; mask > 0; mask >>= 1)
//              {
//                  buffer[len++] = (*(m_memory + dest + n) & mask) ? ' ' : '#';
//              }
//              buffer[len++] = '\0';
//              //printf("%02x ", *(m_memory + dest + n));
//              printf("%s", buffer);
//              if (n % 80 == 0)
//              {
//                printf("\n");
//              }              
        ...
        {
          //printf("WRITE disk: sects = %d = fwrite(0x%02x, 1, %d, %s);\n", sects, dest, count, m_machine->diskFilename(drive));
          if (dest >= 0xA0000 && dest <= 0xBFFFF)
          {
            printf("WRITE VRAM to disk: sects = %d = fwrite(0x%02x, 1, %d, %s);\n", sects, dest, count, m_machine->diskFilename(drive));
            // first copy the bytes from video memory into normal memory for fwrite to use
            for (int n = 0; n < count; n++)
            {
              *(m_memory + dest + n) = m_machine->readVideoMemory8(m_machine, dest + n);
...
//              }            
          ...
          //int starttime = millis();
          sects = fwrite(m_memory + dest, 1, count, m_machine->disk(drive));
          //printf("write took %dms\n", millis() - starttime);
        }
        sects /= 512;
        //printf("sects = %d\n", sects);
      }
...
void BIOS::videoHandlerEntry()
{
  if (i8086::AH() != 0x0c)
  {
    printf("bios.cpp:%d: INT 10h, AX=0x%04x, BX=0x%04x\n", __LINE__, i8086::AX(), i8086::BX());
  }
  //return;
  ...
  switch (i8086::AH()) 
  {
    // set video modes (for EGA and VGA)
    case 0x00:
      switch(i8086::AL())
      {
        case 0x0d:
        {
          printf("INT 10h, set video mode 0x%02x\n", i8086::AL());
          m_machine->setVideoMode(i8086::AL());
          break;
        }
        case 0x0e:
        {
    ...
    // Write Pixel
    case 0x0c:
      m_machine->setPixel(i8086::CX(), i8086::DX(), i8086::AL());
      /*
      switch (ga->emulation()) 
      {
      ...
      */
      //printf("unsupported INT 10 function 0Ch: setPixel %d:%d,%d\n", i8086::CX(), i8086::DX(), i8086::AL());
      break;
    default:
