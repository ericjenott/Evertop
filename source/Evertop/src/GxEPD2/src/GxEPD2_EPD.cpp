...
GxEPD2_EPD::GxEPD2_EPD(int16_t cs, int16_t dc, int16_t rst, int16_t busy, int16_t busy_level, uint32_t busy_timeout,
  ...
  _pSPIx(&SPI), _spi_settings(10000000, MSBFIRST, SPI_MODE0)
{
  _initial_write = true;
...
void GxEPD2_EPD::setParameters(uint32_t serial_diag_bitrate, bool initial, uint16_t reset_duration, bool pulldown_rst_mode)
{
...
void GxEPD2_EPD::writeCommand(uint8_t c)
{
  _writeCommand(c);
}
...
void GxEPD2_EPD::writeData(uint8_t d)
{
  _writeData(d);
}
