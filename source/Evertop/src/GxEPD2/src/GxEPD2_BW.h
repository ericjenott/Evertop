...
class GxEPD2_BW : public GxEPD2_GFX_BASE_CLASS
{
  public:
    ...
    void displayFast(int x, int y, int w, int h)
    {
      //epd2.writeImageFast(_buffer, 0, 0, WIDTH, _page_height);
      //Serial.printf("WIDTH = %d, _page_height = %d\n", WIDTH, _page_height);
      //epd2.writeImageFast(_buffer, 0, 0, WIDTH, _page_height);
      epd2.writeImageFast(_buffer, x, y, w, h);
      //epd2.refreshFast(0, 0, WIDTH, _page_height);
      //epd2.refresh(true);
      //epd2.writeImageAgain(_buffer, 0, 0, WIDTH, _page_height); //need this?
    }
	
	void displayFast2(int x, int y, int w, int h)
    {
      //epd2.writeImageFast(_buffer, 0, 0, WIDTH, _page_height);
      //Serial.printf("WIDTH = %d, _page_height = %d\n", WIDTH, _page_height);
      //epd2.writeImageFast(_buffer, 0, 0, WIDTH, _page_height);
      epd2.writeImageFast2(_buffer, x, y, w, h);
      //epd2.refreshFast(0, 0, WIDTH, _page_height);
      //epd2.refresh(true);
      //epd2.writeImageAgain(_buffer, 0, 0, WIDTH, _page_height); //need this?
    }
    ...
    void displayStayOn(bool partial_update_mode = false)
    {
      if (partial_update_mode)
      {
        //Serial.printf("epd2.writeImage()\n");
        epd2.writeImage(_buffer, 0, 0, WIDTH, _page_height);
      }
      else
      {
        //Serial.printf("epd2.writeImageForFullRefresh()\n");
        epd2.writeImageForFullRefresh(_buffer, 0, 0, WIDTH, _page_height);
      }
      epd2.refresh(partial_update_mode);
      if (epd2.hasFastPartialUpdate)
      {
        //Serial.printf("epd2.writeImageAgain()\n");
        epd2.writeImageAgain(_buffer, 0, 0, WIDTH, _page_height);
      }
      //if (!partial_update_mode) epd2.powerOff();
    }
    ...
    void displayWindowFast(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
    {
      x = gx_uint16_min(x, width());
      y = gx_uint16_min(y, height());
      w = gx_uint16_min(w, width() - x);
      ...
      _rotate(x, y, w, h);
      uint16_t y_part = y;
      epd2.writeImagePartFast(_buffer, x, y_part, WIDTH, _page_height, x, y, w, h);
      //epd2.refreshFast(x, y, w, h);
      // if (epd2.hasFastPartialUpdate)
      // {
        // epd2.writeImagePartAgain(_buffer, x, y_part, WIDTH, _page_height, x, y, w, h);
      // }
