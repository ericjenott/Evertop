...
#include "dispdrivers/epdcontroller.h"
...
InputBox::InputBox(uiApp * app)
  : m_vgaCtrl(nullptr),
    ...
    m_backgroundColor(RGB888(255, 255, 255)),
    m_existingApp(app),
    m_autoOK(0),
...
void InputBox::begin(int viewPortWidth, int viewPortHeight, int displayColors, bool clearScreen)
{
  m_vgaCtrl = new EPDController;
  m_dispCtrl = m_vgaCtrl;
  m_vgaCtrl->begin();
  m_vgaCtrl->setResolution(viewPortWidth, viewPortHeight, clearScreen);

  // setup keyboard and mouse
  if (!PS2Controller::initialized())
  {
    printf("In iBox, re-initializing keyboard and mouse\n");
    PS2Controller::begin(PS2Preset::KeyboardPort0_MousePort1, KbdMode::GenerateVirtualKeys);
  }
  else
  {
    printf("In iBox, keyboard and mouse previously initialized\n");
...
void InputBox::end()
{
  if (m_vgaCtrl) 
  {
    //Serial.println("deleting m_vgaCtrl");
    m_vgaCtrl->end();
    delete m_vgaCtrl;
    m_vgaCtrl = nullptr;
  }
...
void InputBox::exec(InputForm * form)
{
  if (m_existingApp) 
  {
    Serial.print("execing app in existing mode");
  ...
  else 
  {
    ...
    //Serial.println("running app in standalone mode");
    InputApp app(form);
    app.run(m_dispCtrl);
  }
  resetButtons();
...
{
  setupButton(B_CANCEL, buttonCancelText); // I test reversing the order of these two calls to setupButton().  It made no difference as to the buttons' positions or tab indexes.
  ...
  form.autoCancel			= m_autoCancel;
  //printf("form.autoOK = %d\n", form.autoOK);
...
InputResult InputBox::message(char const * titleText, char const * messageText, char const * buttonCancelText, char const * buttonOKText, bool defaultToCancel)
{
  //printf("making CANCEL button\n");
  setupButton(B_CANCEL, buttonCancelText);
  //printf("making OK button\n");
  setupButton(B_OK, buttonOKText);
  //printf("done making buttons\n");
  ...
  form.autoOK               = m_autoOK;
  form.autoCancel			= m_autoCancel;
  printf("form.autoCancel = %d\n", form.autoCancel);
  if (defaultToCancel)
  { 
    // add something here to set focus to button?
  }
  //printf("form.autoOK = %d\n", form.autoOK);
...
int InputBox::menu(char const * titleText, char const * messageText, char const * itemsText, char separator)  // separator defaults to ";"
{
  SelectForm form(this);
  form.titleText            = titleText;
  form.messageText          = messageText;
...
void InputForm::init(uiApp * app_, bool modalDialog_)
{
  ...
  for (int i = 0; i < BUTTONS; ++i) {
    ...
    if (btext) 
    {
      ...
      int buttonExtent = app->canvas()->textExtent(font, btext) + 2;
      ...
      //printf("\"%s\" width = %d, buttonsWidth = %d\n", btext, buttonExtent, buttonsWidth);
      ++totButtons;
    }
  }
  ...
  if (totButtons) 
  {
    ...
    for (int i = 0; i < BUTTONS; ++i)
    {
      if (inputBox->buttonText(i)) 
      {
        ...
        else 
        {
          ...
          {
            retval = (InputResult)(i + 1);
            //printf("in button->onClick(), i = %d, retval = %d, B_CANCEL = %d, B_OK = %d\n", i, retval, B_CANCEL, B_OK);
            finalize();
          };
          ctrl = button;
        }
        ...
        if(!controlToFocus)
        {
          //printf("set controlToFocus\n");
          controlToFocus = ctrl;   // without this "if" clause, this line would automatically give the focus to the button with the highest index.  Not good.
        }
        else
        {
          //printf("skipping controlToFocus\n");
        }
      }
    }
    ...
    if (autoOK > 0) 
    {
      //printf("inside if (autoOK > 0) : autoOK = %d\n", autoOK);
      ...
      mainFrame->onTimer = [&](uiTimerHandle t) {
        ...
        if (app->lastUserActionTime() + 900 > now) 
        {
          ...
          //printf("returning early from mainFrame->onTimer due to app->lastUserActionTime() + 900 > now\n");
          return;
        }
        if (autoOK <= 0) 
        {
        ...
        --autoOK;
        //printf("setting autoOK label\n");
        autoOKLabel->setTextFmt("%d", autoOK);
      };
      //printf("app->setTimer(mainFrame, 1000)\n");
...
{
  //printf("inside InputForm::defaultEnterHandler\n");
  if (key.VK == VK_RETURN || key.VK == VK_KP_ENTER) 
  {
    //printf("inside if (key.VK == VK_RETURN || key.VK == VK_KP_ENTER) in InputForm::defaultEnterHandler\n");
    retval = InputResult::Enter;
    finalize();
  }
  else
  {
    //printf("doing NOTHING in InputForm::defaultEnterHandler\n");
  }
}
...
{
  //printf("inside InputForm::defaultEscapeHandler()\n");
  if (key.VK == VK_ESCAPE) 
  {
    //printf("inside if (key.VK == VK_ESCAPE) in InputForm::defaultEscapeHandler()\n");
    retval = InputResult::Cancel;
    finalize();
  }
  else
  {
    //printf("did NOTHING in InputForm::defaultEscapeHandler()\n");
  }
  //printf("delaying 200ms for debug testing...\n");
  //vTaskDelay(200);
}
...
{
  //printf("inside InputForm::doExit(int value), value = %d\n", value);
  if (modalDialog)
  {
    //printf("mainFrame->exitModal(value), value = %d\n", value);
    mainFrame->exitModal(value);
  }
  else 
  {
    //printf("app->quit(value), value = %d\n", value);
    app->quit(value);
    // this avoids flickering of content painted in onPaint
    //app->rootWindow()->frameProps().fillBackground = false;
    app->rootWindow()->frameProps().fillBackground = true;
  }
}
...
void TextInputForm::addControls()
{
  ...
  edit->onKeyType = [&](uiKeyEventInfo const & key) 
  { 
    //printf("onKeyType delegate in TextInputForm\n");
    defaultEnterHandler(key); 
    defaultEscapeHandler(key); 
  };
...
void MessageForm::addControls()
{
  ...
  { 
    //printf("onKeyUp in MessageForm, NOT calling InputForm::defaultEnterHandler(key)\n");
    //defaultEnterHandler(key);
    defaultEscapeHandler(key); 
  };
  //setFocusedWindow(m_button);
  //printf("calling app->setFocusedWindow(controlToFocus)\n");
  //app->setFocusedWindow(controlToFocus);
  //mainFrame->setFocusIndex(2);       //USELESS. testing, trying to change default button index
}


void MessageForm::finalize()
{
  //printf("inside MessageForm::finalize()\n");
  doExit(0);
}
...
void SelectForm::calcRequiredSize()
{
  ...
  //printf("requiredWidth = %d\n", requiredWidth);
  requiredWidth           = imax(requiredWidth, (int) (maxLength * app->canvas()->textExtent(font, "M")) + 20);  // this seems to be a bug.  Should find the pixel width of the longest list box "item", not the letter "M".  WIP?
  //printf("requiredWidth = %d\n", requiredWidth);
...
void SelectForm::addControls()
{
  ...
  y += font->height + 6;
  //printf("listBox = new uiListBox(mainFrame, Point(%d, %d), Size(%d - 10, %d));\n", mainFrame->clientSize().width, listBoxHeight);
  //Serial.println("mainFrame->clientSize().width = %d", mainFrame->clientSize().width);
  listBox = new uiListBox(mainFrame, Point(x, y), Size(mainFrame->clientSize().width - 10, listBoxHeight));
  //printf("listBox created successfully\n");
  ...
  { 
    //printf("onKeyType delegate in SelectForm\n");
    defaultEnterHandler(key); 
    defaultEscapeHandler(key); 
  };

  controlToFocus = listBox;
  // printf("listBox->firstSelectedItem() = %d\n", listBox->firstSelectedItem());
  if (listBox->firstSelectedItem() == -1)
  {
    listBox->selectItem(0);  //this forces default selection and highlight to be the first item, which isn't necessarily what we want
  }
}
...
void ProgressForm::addControls()
{
  ...
  mainFrame->onKeyUp = [&](uiKeyEventInfo const & key) 
  { 
    //printf("onKeyUp delegate in ProgressForm\n");
    defaultEscapeHandler(key); 
  };
}
...
void FileBrowserForm::addControls()
{
  ...
  mainFrame->onKeyUp = [&](uiKeyEventInfo const & key) 
  { 
    //printf("onKeyUp delegate in FileBrowserForm\n");
    defaultEscapeHandler(key); 
  };
...
void FileSelectorForm::addControls()
{
  ...
  mainFrame->onKeyUp = [&](uiKeyEventInfo const & key)
  { 
    //printf("onKeyUp delegate in FileSelectorForm\n");
    defaultEscapeHandler(key); 
  };
  ...
  fileBrowser->onKeyType = [&](uiKeyEventInfo const & key) 
  { 
    //printf("onKeyType delegate in FileSelectorForm\n");
    defaultEnterHandler(key); 
    defaultEscapeHandler(key); 
  };
