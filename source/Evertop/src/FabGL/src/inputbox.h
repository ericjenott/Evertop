...
struct FileSelectorForm : public InputForm {
  ...
  static constexpr int MINIMUM_EDIT_WIDTH  = 160;  //enough for 20 characters (8pix per character)
...
public:
   ...
   */
  void begin(int viewPortWidth = -1, int viewPortHeight = -1, int displayColors = 16, bool clearScreen = true);
   ...
   */
  void setAutoOK(int timeout, bool cancel = 0)  { m_autoOK = timeout; m_autoCancel = cancel;}
   ...
   * @param buttonOKText Optional text for OK button (nullptr = hasn't OK button). Default is "OK".
   * @param defaultToCancel Optional set default button to CANCEL button. Default is false (defaults to OK button).
   *
   * @return Dialog box result (Cancel or Enter)
   *
   * Example:
   ...
   *     ib.end();
   */
  InputResult message(char const * titleText, char const * messageText, char const * buttonCancelText = nullptr, char const * buttonOKText = "OK", bool defaultToCancel = false);
...
private:
  ...
  bool								  m_autoCancel;								   // Cancel instead of Enter on AutoOK
