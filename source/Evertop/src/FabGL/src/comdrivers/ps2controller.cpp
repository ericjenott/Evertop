...
{
  //printf("inside PS2Controller::begin(gpio_num_t port0_clkGPIO, gpio_num_t port0_datGPIO, gpio_num_t port1_clkGPIO, gpio_num_t port1_datGPIO)\n");
  // ULP stuff is always active, even when end() is called
  if (!s_initDone) 
  {
    //printf("if (!s_initDone)\n");
    ...
    if (s_portEnabled[0]) {
      ...
      {
        printf("Invalid PS/2 Port 0 pins\n");
    ...
    if (s_portEnabled[1]) {
      if (!rtc_gpio_is_valid_gpio(port1_clkGPIO) || !rtc_gpio_is_valid_gpio(port1_datGPIO)) {
        //printf("Invalid PS/2 Port 1 pins\n");
...
{
  printf("inside PS2Controller::begin(PS2Preset preset, KbdMode keyboardMode)\n");
  end();
  ...
  switch (preset) {
    case PS2Preset::KeyboardPort0_MousePort1:
      printf("inside case PS2Preset::KeyboardPort0_MousePort1:\n");
      // both keyboard (port 0) and mouse configured (port 1)
      printf("calling PS2Controller.begin(GPIO_NUM_33, GPIO_NUM_32, GPIO_NUM_26, GPIO_NUM_27)\n");
...
{
  //printf("PS2Con:%d\n", uxQueueMessagesWaiting(s_dataIn[PS2Port]));
  // afer light sleep, uxQueueMessagesWaiting(s_dataIn[PS2Port]) always returns 0, breaking keyboard input
...
{
  // this never runs again after wakeup from light sleep
  //if (millis() % 10 == 0)  Serial.printf(".");
  int r = -1;
...
{
  // this no longer runs after light sleep wakeup
  ...
  if (rtc_intr & RTC_CNTL_SAR_INT_ST) 
  {
    for (int p = 0; p < 2; ++p) 
    {
      if (RTC_SLOW_MEM[RTCMEM_PORT0_RX + p] & 0xffff) 
      {
        ...
        RTC_SLOW_MEM[RTCMEM_PORT0_RX + p] = 0;
        //Serial.printf("@");  // get this if press a key
      } 
      ...
      //Serial.printf(";"); // get two
    }
    //Serial.printf(","); // get one
  }
