...
{
  //printf("PS2:%d\n", PS2Controller::dataAvailable(m_PS2Port));
...
bool PS2Device::send_cmdLEDs(bool numLock, bool capsLock, bool scrollLock)
{
  ...
  //printf("send_cmdLEDs(%d, %d, %d)\n", (int)numLock, (int)capsLock, (int)scrollLock);  
...
bool PS2Device::cmdDisableScanning()
{
  PS2DeviceLock deviceLock(this);
  PS2DeviceType result = PS2DeviceType::UnknownPS2Device;
  return send_cmdDisableScanning();
}


bool PS2Device::cmdEnableScanning()
{
  return send_cmdEnableScanning();
}
...
bool PS2Device::send_cmdReset()
{
  ...
  {
    Serial.printf("sendCommand returned false\n");
    return false;
  }
  int d = getData(500);
  printf("getData(500) returned 0x%02x\n", d);
  return d == PS2_REPLY_SELFTEST_OK;
