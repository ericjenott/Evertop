...
uint32_t BREAKs = 0;
uint32_t FLOWSs = 0;
uint32_t suspendRXs = 0;
uint32_t resumeRXs = 0;
uint32_t WHILEs = 0;
uint32_t FLOWs2 = 0;
uint32_t suspendRXs2 = 0;
uint32_t READs = 0;
uint32_t XONXOFFs = 0;
uint32_t RXCALLBACKs = 0;
...
int SerialPort::uartGetRXFIFOCount()
{
  ...
  //m_dev->mem_rx_status.wr_addr != m_dev->mem_rx_status.rd_addr
  int len = 0;
  if (m_dev->mem_rx_status.wr_addr > m_dev->mem_rx_status.rd_addr)
  {
    len = m_dev->mem_rx_status.wr_addr - m_dev->mem_rx_status.rd_addr;
  }
  else if (m_dev->mem_rx_status.wr_addr < m_dev->mem_rx_status.rd_addr)
  {
    len = (m_dev->mem_rx_status.wr_addr + 128) - m_dev->mem_rx_status.rd_addr;
  }
  else
  {
    len = m_dev->status.rxfifo_cnt > 0 ? 128 : 0;
  }
  return len;
...
{
  printf("SerialPort::setup(%d) running on core ", uartIndex);
  printf("%d\n", xPortGetCoreID());
  if (!m_initialized) {
    ...
    //esp_intr_alloc_pinnedToCore(INTR_SRC[m_idx], 0, uart_isr, this, nullptr, CoreUsage::quietCore() ^ 1);
    esp_intr_alloc_pinnedToCore(INTR_SRC[m_idx], 0, uart_isr, this, nullptr, xPortGetCoreID());
    printf("set uart_isr ISR to run on core %d with flag 0\n", xPortGetCoreID());
...
{
  printf("setBaud(%d), numISRs = %d, numOVFs = %d, BREAKs = %d, FLOWs = %d, suspendRXs = %d, resumeRXs = %d, WHILEs = %d, FLOWs2 = %d, suspendRXs2 = %d, READs = %d, XONXOFFs = %d, RXCALLBACKs = %d\n", value, numISRs, numOVFs, BREAKs, FLOWSs, suspendRXs, resumeRXs, WHILEs,  FLOWs2,  suspendRXs2,  READs,  XONXOFFs,  RXCALLBACKs);
  // the above printout shows that I receive all the bytes I send, even though they are lost before displaying.  Seems they aren't making it into the FIFO buffer, or are being erased when any of the bytes are read out.
  numISRs = 0;
  numOVFs = 0;
  FLOWSs = 0;
  suspendRXs = 0;
...
void SerialPort::uart_isr(void * arg)
{
  ...
  // is this getting run eroneously and flushing FIFO?  No, numOVRs is 0 when numISRs is 124
  if (dev->int_st.rxfifo_ovf || dev->int_st.frm_err || dev->int_st.parity_err) 
  {
    ...
    if (ser->m_lineStatusCallback)  // in our case this is false, won't go in here.
    {
  ...
  //FLOWs, suspendRXs, and resumeRXs are all 0, no trouble above.

  // main receive loop
  
  /*
  https://www.espressif.com/sites/default/files/documentation/esp32_errata_en.pdf
  3.17. UART fifo_cnt does not indicate the data length in FIFO
  correctly.
  Details:
  When software uses DPORT to read UART fifo_cnt, and such operation is interrupted, then
  fifo_cnt will decrement by 1 erroneously.
  Workarounds:
  When using DPort to read fifo, calculate the real count based on the FIFO read and write
  offset address.
  For example,
    if (wr_addr > rd_addr)
    {
  ...
  in the above code snippet, wr_addr represents the FIFO write offset address, rd_addr
  represents the FIFO read offset address, fifi_cnt represents the number of valid bytes in
  the FIFO, len represents the correct number of valid bytes after calculation
  */
  ...
  while (ser->uartGetRXFIFOCount() != 0 || dev->mem_rx_status.wr_addr != dev->mem_rx_status.rd_addr) 
  {
    ...
    //if (ser->m_flowControl != FlowControl::None && !ser->m_rxReadyCallback(ser->m_callbackArgs, true)) // if flow control is off and PC8250::rxReadyCallback returns false (it never will)
    if (ser->m_flowControl != FlowControl::None) // if flow control is off and 
    {
      FLOWs2++;
      if (!ser->m_rxReadyCallback(ser->m_callbackArgs, true)) // if PC8250::rxReadyCallback returns false  (it never will)
      {
        ser->flowControl(false);  // suspend RX
        suspendRXs2++;
