...
void i8042::init()
{
  ...
  {
    printf("calling m_PS2Controller.begin(PS2Preset::KeyboardPort0_MousePort1, KbdMode::NoVirtualKeys);\n");
    m_PS2Controller.begin(PS2Preset::KeyboardPort0_MousePort1, KbdMode::NoVirtualKeys);  //keyboard on PS/2 port 0, mouse on port 1
    //m_PS2Controller.begin(PS2Preset::KeyboardPort1_MousePort0, KbdMode::NoVirtualKeys);  //keyboard on PS/2 port 0, mouse on port 1
    
    Serial.printf("m_PS2Controller.begin\n");
    //m_PS2Controller.begin(PS2Preset::KeyboardPort0, KbdMode::NoVirtualKeys);  //keyboard only, on PS/2 port 0
  }
  else
  {
    printf("PS2Controller already init'ed, calling m_PS2Controller.keyboard()->enableVirtualKeys(false, false)\n");
    m_PS2Controller.keyboard()->enableVirtualKeys(false, false);
  }
  m_keyboard = m_PS2Controller.keyboard();
  m_mouse    = m_PS2Controller.mouse();  //uncomment if using mouse port
...
void i8042::reset()
{
  ...
  // for keyboard only:
  //m_commandByte = CMDBYTE_ENABLE_KEYBOARD_IRQ | CMDBYTE_SYSFLAG | CMDBYTE_STD_SCAN_CONVERSION;
...
void i8042::tick()
{
  ...
  m_sysReqTriggered = false;    
    ...
    */
  // still runs after wakeup from light sleep
  //printf(".");
  
  /* Normal working keyboard situation while holding down a key:
    21:04:52.289 -> 1(m_commandByte & CMDBYTE_DISABLE_KEYBOARD) = 0
    21:04:52.289 -> (m_STATUS & STATUS_OBF) = 0

  */
    
  /* after light sleep wakeup while holding down a key:
    21:03:10.360 -> 0(m_commandByte & CMDBYTE_DISABLE_KEYBOARD) = 0
    21:03:10.360 -> (m_STATUS & STATUS_OBF) = 0
  */
  
  // above proves that we never have m_keyboard->scancodeAvailable() = true after light sleep wakeup
  // printf("(m_STATUS & STATUS_OBF) = %d\n", (int)(m_STATUS & STATUS_OBF));
  // printf("%d",(int)(m_keyboard->scancodeAvailable()));
  // printf("(m_commandByte & CMDBYTE_DISABLE_KEYBOARD) = %d\n", (int)(m_commandByte & CMDBYTE_DISABLE_KEYBOARD));
  
  //in the following if statement, m_keyboard->scancodeAvailable() is false:
  ...
  if ((m_STATUS & STATUS_OBF) == 0 && m_keyboard->scancodeAvailable() && (m_commandByte & CMDBYTE_DISABLE_KEYBOARD) == 0) 
  {
    ...
    {
      // never runs after wakeup from light sleep
      //Serial.printf(";\n");
      // this is always used in the 8086 VM
  ...
  {
    //printf("something to execute (with parameters)\n");
    m_STATUS &= ~STATUS_IBF;
    execCommand();
  }
  ...
  {
    //printf("m_keybIntTrigs && trigKeyboardInterrupt()\n");
    --m_keybIntTrigs;
  }
...
void i8042::execCommand()
{
  ...
  switch (cmd) {
    ...
    case CTRLCMD_TEST_KEYBOARD_PORT:
      //printf("inside case CTRLCMD_TEST_KEYBOARD_PORT:\n");
...
void i8042::checkSysReq(int scode2)
{
  if (m_DBBOUT == 0xf0)
  {
    ...
    //if (scode2 == 0x7E) // Scrolllock released? 0x7E is VK_SCROLLLOCK  
    
    /* 
        Look in devdrivers/kbdlayouts.cpp for key codes
    */
    if (scode2 == 0x84) // SysReq released?  
    { 
      m_sysReqTriggered = true;
    }
    else if (m_sysReqTriggered && scode2 == 0x11)   // ALT released? alt+printscreen = total refresh screen
    {
      m_sysReqTriggered = false;
      m_sysReq2(m_context);
    }
    else if (m_sysReqTriggered && scode2 == 0x14)   // LCTRL released? lctrl + alt + printscreen = invert screen b/w
    {
      m_sysReqTriggered = false;
      m_sysReq4(m_context);
    }
    else if (m_sysReqTriggered && scode2 == 0x12)   // LSHIFT released? lshift + alt + printscreen = hibernate to disk
    {
      m_sysReqTriggered = false;
      m_sysReq5(m_context);
    }
    else if (m_sysReqTriggered && scode2 == 0x59)   // WINDOWS KEY released? WINDOWS KEY + alt + printscreen = unhibernate from disk
    {
      m_sysReqTriggered = false;
      m_sysReq6(m_context);
    }
    //0x1f is "windows" key, 0x2f is "menu" key
    else if (scode2 == 0x1f) // windows key = quick refresh screen
    {
      m_sysReqTriggered = false;
      m_sysReq(m_context);
    }
    else if (scode2 == 0x2f) // menu key = invert fg/bg b/w
    {
      m_sysReqTriggered = false;
      m_sysReq3(m_context);
    }
...
uint8_t *i8042::get_m_STATUS(){return &m_STATUS;}
uint8_t *i8042::get_m_DBBOUT(){return &m_DBBOUT;}
uint8_t *i8042::get_m_DBBIN(){return &m_DBBIN;}
uint8_t *i8042::get_m_commandByte(){return &m_commandByte;}
bool *i8042::get_m_writeToMouse(){return &m_writeToMouse;}

// disable keyboard when hibernating
// bool i8042::disableKeyboard()
// {
  // Serial.println("m_keyboard->cmdDisableScanning();\n");
  // m_keyboard->cmdDisableScanning();
// }

// bool i8042::enableKeyboard()
// {
  // Serial.println("m_keyboard->cmdEnableScanning();\n");
  // m_keyboard->cmdEnableScanning();
// }  

// void i8042::addKeybIntTrigs()
// {
  // //int scode2 = m_keyboard->getNextScancode();
  // int scode2 = 13;
  // checkSysReq(scode2);
