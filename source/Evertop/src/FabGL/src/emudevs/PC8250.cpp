...
{
  printf("PC8250::setSerialPort() running on core ");
  printf("%d\n", xPortGetCoreID());
  m_serialPort = value;
...
{
  // this gets called from "SerialPort" in serialport.cpp's uart_isr when a byte is received.  
  // It sets the 8250's m_dataRecv = value of received byte
  // and m_dataReady = true, both of which variables will affect the next tick() cycle
  ...
  switch(reg & 7) {
    ...
    case 0:
      ...
      else 
      {
        ...
        ret = m_MCR & PC8250_MCR_LOOPBACK ? m_THR : m_dataRecv;  //wtf? that's it?  No FIFO buffer?  If don't read the byte this time around, it's gone!
        m_dataReady = false;
...
{
  if (m_dataReady && (m_MCR & PC8250_MCR_LOOPBACK) == 0)  // if byte received and not in loopback mode
  {
    m_LSR   |= PC8250_LSR_DR;                // set DR flag
    m_trigs |= m_IER & PC8250_IER_RX_INT;    // set int triggered flag
    //printf("cbr(): m_dataReady\n");
  }
}
...
void PC8250::tick()
{
  ...
  if (m_IER) 
  {
    ...
    {
      //printf("m_trigs. OVERRUNs= %d, RXCALLBACKs8250 = %d\n", OVERRUNs, RXCALLBACKs8250);
      if (m_interruptCallback(this, m_context)) 
      {
        // this doesn't happen when updating screen.  One way to fix is to make sure that this process doesn't block while screen is being updated
        // another (bad) way to fix is to create a FIFO buffer, and keep on generating dataReady/dataRcvd interrupts until the buffer is empty
        //printf("INTR\n");
        #if PC8250_DEBUG
        printf("INTR\n");
        #endif
