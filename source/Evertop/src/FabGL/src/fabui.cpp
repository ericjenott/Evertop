...
int uiApp::run(BitmappedDisplayController * displayController, Keyboard * keyboard, Mouse * mouse)
{
  ...
  if (PS2Controller::initialized()) 
  {
    ...
    if (m_mouse == nullptr)
    {
      //printf("m_mouse == nullptr, calling PS2Controller::mouse()\n");
      m_mouse = PS2Controller::mouse();
    }
  }

  //m_eventsQueue = xQueueCreate(FABGLIB_UI_EVENTS_QUEUE_SIZE, sizeof(uiEvent));
  // instead of using the heap, use the same memory location every time in PSRAM
  #define QUEUE_LENGTH    FABGLIB_UI_EVENTS_QUEUE_SIZE
  #define ITEM_SIZE       sizeof(uiEvent)
  static StaticQueue_t xStaticQueue;
  //printf("ucQueueStorageArea occupies %d bytes\n", QUEUE_LENGTH * ITEM_SIZE);
  ...
  m_eventsQueue = xQueueCreateStatic( QUEUE_LENGTH, ITEM_SIZE, ucQueueStorageArea, &xStaticQueue );
  if (!m_eventsQueue)
  {
    //Serial.println("FAILED CREATING m_eventsQueue");
  }
  ...
  if (m_mouse && m_mouse->isMouseAvailable())
  {
    //Serial.println("calling m_displayController->setMouseCursor()");
  ...
  //m_displayController->enableBackgroundPrimitiveTimeout(true);
  ...
  //Serial.println("Deleting m_eventsQueue");
  ...
  if (event->dest == nullptr) 
  {
    ...
    switch (event->id) 
    {
      ...
      case UIEVT_KEYDOWN:
        isKeyDown = true;
      case UIEVT_KEYUP:
        //printf("%s in uiApp::preprocessEvent()\n", isKeyDown ? "UIEVT_KEYDOWN" : "UIEVT_KEYUP" );
...
void uiApp::preprocessKeyboardEvent(uiEvent * event)
{
  ...
  {
    //printf("UIEVT_KEYDOWN in uiApp::preprocessKeyboardEvent\n");
    m_keyDownHandler = event->dest;
  }
  else if (event->id == UIEVT_KEYUP && m_keyDownHandler == event->dest) 
  {
    //printf("UIEVT_KEYUP in uiApp::preprocessKeyboardEvent()\n");
    // can't skip this to fix double select bug, or else can never select anthing else in other forms
    if (m_override_keyDownHandler)
    {
      //printf("skipping creation of UIEVT_KEYTYPE event since was triggered by UIEVT_KEYUP in file browser\n");
      m_override_keyDownHandler = false;
    }
    else
    {
      //printf("creating uiEvent(event->dest, UIEVT_KEYTYPE) in preprocessKeyboardEvent\n");
...
bool uiApp::postEvent(uiEvent const * event)
{
  if (!m_eventsQueue)
  {
    Serial.println("m_eventsQueue DOESN'T EXIST!");
  }
  //Serial.println("#");
...
uiTimerHandle uiApp::setTimer(uiEvtHandler * dest, int periodMS)
{
  ...
  xTimerStart(h, 0);
  printf("created event timer periodMS = %d\n", periodMS);
  return h;
}
...
uiMessageBoxResult uiApp::messageBox(char const * title, char const * text, char const * button1Text, char const * button2Text, char const * button3Text, uiMessageBoxIcon icon)
{
  ...
  panel->windowStyle().borderColor = RGB888(0, 0, 0);
...
uiMessageBoxResult uiApp::inputBox(char const * title, char const * text, char * inOutString, int maxLength, char const * button1Text, char const * button2Text)
{
  ...
  mainFrame->onKeyUp = [&](uiKeyEventInfo const & key) 
  {
    ...
    {
      printf("inside uiApp::inputBox's mainFrame->onKeyUp delegate, key.VK is VK_RETURN or VK_KP_ENTER\n"); 
      printf("calling mainFrame->exitModal(1)\n");
      mainFrame->exitModal(1);
    }
    else if (key.VK == VK_ESCAPE)
    {
      printf("inside uiApp::inputBox's mainFrame->onKeyUp delegate, key.VK is VK_ESCAPE\n"); 
      mainFrame->exitModal(0);
      printf("calling mainFrame->exitModal(0)\n");
    }
  };
...
uiMessageBoxResult uiApp::fileDialog(char const * title, char * inOutDirectory, int maxDirNameSize, char * inOutFilename, int maxFileNameSize, char const * buttonOKText, char const * buttonCancelText, int frameWidth, int frameHeight)
{
  ...
  auto filenameEdit = new uiTextEdit(mainFrame, inOutFilename, Point(x + 72 + fnBorder, y), Size(frameWidth - x - 80 - fnBorder * 2, hh));
  ...
  mainFrame->onKeyUp = [&](uiKeyEventInfo const & key) 
  {
    if (key.VK == VK_RETURN || key.VK == VK_KP_ENTER)
    {
      //printf("inside uiApp::fileDialog's mainFrame->onKeyUp, key.VK is VK_RETURN or VK_KP_ENTER\n"); 
      //printf("browser->m_override_keyDownHandler = %d\n", browser->m_override_keyDownHandler);
      
      if (browser->m_override_keyDownHandler)
      {
        browser->m_override_keyDownHandler = false;
        //printf("SKIPPING mainFrame->exitModal(1) because VK_RETURN/VK_KP_ENTER event was generated from filebrowser control to enter subdirectory\n");
      }
      else
      {
        // this is a bug.  If directory is selected, should enter subdir and keep fileDialog modal open.
        mainFrame->exitModal(1);
      }
    }
    else if (key.VK == VK_ESCAPE)
    {
      //printf("inside uiApp::fileDialog's mainFrame->onKeyUp, key.VK is VK_ESCAPE\n");
      mainFrame->exitModal(0);
    }
  };  
...
void uiWindow::processEvent(uiEvent * event)
{
  ...
  switch (event->id) {
    ...
    case UIEVT_KEYDOWN:
      //printf("UIEVT_KEYDOWN in uiWindow::processEvent\n");
      if (m_parentProcessKbdEvents)
      {
        //printf("UIEVT_KEYDOWN, calling m_parent->processEvent(UIEVT_KEYDOWN) in uiWindow::processEvent\n");
        m_parent->processEvent(event);
      }
      break;

    case UIEVT_KEYUP:
      //printf("UIEVT_KEYUP in uiWindow::processEvent()\n");
      if (m_parentProcessKbdEvents)
      {
        m_parent->processEvent(event);
      }
...
void uiFrame::processEvent(uiEvent * event)
{
  ...
  switch (event->id) {
    ...
    case UIEVT_KEYDOWN:
      // move focused child
      //printf("UIEVT_KEYDOWN in uiFrame::processEvent\n");
      ...
      //printf("calling onKeyDown() in uiFrame::processEvent(\n");
      onKeyDown(event->params.key);
      break;

    case UIEVT_KEYUP:
      //printf("UIEVT_KEYUP in uiFrame::processEvent(), calling onKeyUp(event->params.key)\n");
...
void uiButton::processEvent(uiEvent * event)
{
  ...
  switch (event->id) {
    ...
    case UIEVT_KEYUP:
      //printf("UIEVT_KEYUP in uiButton::processEvent()\n");
      if (event->params.key.VK == VK_RETURN || event->params.key.VK == VK_KP_ENTER || event->params.key.VK == VK_SPACE) 
      {
        trigger();
        //printf("keyup key is ENTER OR SPACE, running onClick() after UIEVT_KEYUP in uiButton::processEvent\n");
        onClick();
      }
      break;
...
void uiTextEdit::processEvent(uiEvent * event)
{
  ...
  switch (event->id) {
    ...
    case UIEVT_KEYDOWN:
      //printf("UIEVT_KEYDOWN in uiTextEdit::processEvent\n");
...
uint8_t const * uiTextEdit::getCharInfo(char ch, int * width)
{
  ...
  if (m_textEditStyle.textFont->chptr) {
    ...
    *width = *chptr++;
    ...
    // fixed width
    //chptr = m_textEditStyle.textFont->data + ch;  //results in garbage characters
    chptr = m_textEditStyle.textFont->data; //should be okay
...
void uiTextEdit::paintContent()
{
  ...
  for (int x = m_contentRect.X1 + m_viewX, y = m_contentRect.Y1, col = 0, fontWidth; m_text[col]; ++col, x += fontWidth) {
    ...
    if (x >= m_contentRect.X1 && x <= m_contentRect.X2)
    {
      ...
      canvas()->drawGlyph(x, y, fontWidth, m_textEditStyle.textFont->height, chptr , m_text[col]); //getCharInfo() now fixed to return correct chptr for fixed width fonts
    }
  }
}
...
void uiCustomListBox::processEvent(uiEvent * event)
{
  ...
  switch (event->id) {
    ...
    case UIEVT_KEYDOWN:
      //printf("UIEVT_KEYDOWN in uiCustomListBox::processEvent\n");
      handleKeyDown(event->params.key);
      break;

    case UIEVT_KEYUP:
      //printf("UIEVT_KEYUP in uiCustomListBox::processEvent(), calling onKeyUp(event->params.key)\n");
      onKeyUp(event->params.key);
      break;

    case UIEVT_KEYTYPE:
      //printf("UIEVT_KEYTYPE in uiCustomListBox::processEvent\n");
      if (m_override_keyDownHandler)
      {
        m_override_keyDownHandler = false;
        //printf("SKIPPING onKeyType because UIEVT_KEYTYPE event was generated from filebrowser control\n");
      }
      else
      {
        //printf("running onKeyType()\n");
        onKeyType(event->params.key);
      }
      break;
...
void uiFileBrowser::enterSubDir()
{
  if (m_selected >= 0) 
  {
    ...
    if (selItem->isDir) 
    {
      ...
      onChange();
      repaint();
      //printf("finished repaint() in uiFileBrowser::enterSubDir()\n");
    }
  }
}
  ...
  switch (event->id) {
    ...
    case UIEVT_KEYDOWN:
      if (event->params.key.VK == VK_RETURN)
      {
        ...
        if (selItem->isDir)
        {
          //printf("UIEVT_KEYDOWN in uiFileBrowser::processEvent, running enterSubDir()\n");
          //printf("setting m_override_keyDownHandler = true to avoid double enter/select bug\n");
          m_override_keyDownHandler = true;
          enterSubDir();
        }
      }
      ...
      {
        //printf("DOING NOTHING WITH UIEVT_KEYDOWN in uiFileBrowser::processEvent\n");
      }  
      break;

    case UIEVT_KEYUP:
      //printf("UIEVT_KEYUP in uiFileBrowser::processEvent(), doing nothing\n");
      //printf("UIEVT_KEYUP in uiFileBrowser::processEvent(), setting m_override_keyDownHandler = true to avoid double enter/select bug\n");
      //m_override_keyDownHandler = true;
      break;
...
void uiCustomComboBox::processEvent(uiEvent * event)
{
  ...
  switch (event->id) {
    ...
    case UIEVT_CREATE:
      ...
      listbox()->onKeyType = [&](uiKeyEventInfo const & key) {
        if (key.VK == VK_TAB || key.VK == VK_RETURN) {
          ...
          app()->postEvent(&evt);
          evt.id = UIEVT_KEYUP;
          //printf("setting evt.id = UIEVT_KEYUP in uiCustomComboBox::processEvent()\n");
          app()->postEvent(&evt);
        }
      };
    ...
    case UIEVT_KEYDOWN:
      //printf("UIEVT_KEYDOWN in uiCustomComboBox::processEvent\n");
      listbox()->processEvent(event);
      break;

    case UIEVT_KEYUP:
      //printf("UIEVT_KEYUP in uiCustomComboBox::processEvent()\n");
...
void uiCheckBox::processEvent(uiEvent * event)
{
  ...
  switch (event->id) {
    ...
    case UIEVT_KEYUP:
      //printf("UIEVT_KEYUP in uiCheckBox::processEvent()\n");
      if (event->params.key.VK == VK_RETURN || event->params.key.VK == VK_KP_ENTER || event->params.key.VK == VK_SPACE) {
        trigger();
        //printf("running onClick() after UIEVT_KEYUP in uiCheckBox::processEvent\n");
        onClick();
      }
      break;
    ...
    m_ticksFrequency(25)
{
  ...
  windowStyle().borderColor        = RGB888(0, 0, 0);
...
void uiSlider::processEvent(uiEvent * event)
{
  ...
  switch (event->id) {
    ...
    case UIEVT_KEYDOWN:
      //printf("UIEVT_KEYDOWN in uiSlider::processEvent\n");
      handleKeyDown(event->params.key);
      break;
...
void uiSimpleMenu::processEvent(uiEvent * event)
{
  ...
  switch (event->id) {
    ...
    case UIEVT_KEYUP:
      //printf("UIEVT_KEYUP in uiSimpleMenu::processEvent()\n");
      if (event->params.key.VK == VK_RETURN || event->params.key.VK == VK_KP_ENTER || event->params.key.VK == VK_SPACE) {
