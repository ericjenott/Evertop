...
void Mouse::begin(int PS2Port)
{
  ...
  {
    printf("s_quickCheckHardware is true, running single PS2Device::quickCheckHardware()\n");
...
bool Mouse::reset()
{
  if (s_quickCheckHardware) 
  {
    printf("s_quickCheckHardware is true, running single send_cmdReset()\n");
    m_mouseAvailable = send_cmdReset();
  } 
  else 
  {
    //printf("s_quickCheckHardware is false, running three send_cmdReset()s\n");
    // tries up to three times for mouse reset
    for (int i = 0; i < 1; ++i)   // temporarily set to 1.  Should be 3 if mouse enabled in BIOS
    {
      m_mouseAvailable = send_cmdReset();
      if (m_mouseAvailable)
      {
  ...
  printf("m_mouseAvailable = %d\n", (int)m_mouseAvailable);
  ...
  if (m_mouseAvailable) 
  {
    ...
    {
      // Intellimouse ok!
      printf("Intellimouse ok! (three buttons + scroll wheel, 4 bytes packet)\n");
      m_mouseType = Intellimouse;
    }
    else
    {
      printf("non-Intellimouse\n");
    }
    setSampleRate(60);
  }
