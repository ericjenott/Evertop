...
//int            FileBrowser::s_SDCardMaxFreqKHz = 40000;
int            FileBrowser::s_SDCardMaxFreqKHz = SDMMC_FREQ_DEFAULT;
//int            FileBrowser::s_SDCardMaxFreqKHz = 50000;
//int            FileBrowser::s_SDCardMaxFreqKHz = 4000;
//int              FileBrowser::s_SDCardMaxFreqKHz = SDMMC_FREQ_DEFAULT;
...
bool FileBrowser::mountSDCard(bool formatOnFail, char const * mountPath, size_t maxFiles, int allocationUnitSize, int MISO, int MOSI, int CLK, int CS)
{
  ...
  // printf("calling esp_vfs_fat_sdspi_mount()\n");
  // //s_SDCardMounted = (esp_vfs_fat_sdspi_mount(mountPath, &host, &slot_config, &mount_config, &s_SDCard) == ESP_OK);

  // SPI.begin(slot_config.gpio_sck, slot_config.gpio_miso, slot_config.gpio_mosi, slot_config.gpio_cs);
  // bool has_SD = false;
  ...
  // if (!SD.begin(slot_config.gpio_cs, SPI, 4000000, "/sd", maxFiles, formatOnFail))
  // { 
    // Serial.println("FATFS via SD.h SD card initialization failed!");
  // }
  // else
  // {
    // Serial.println("FATFS via SD.h SD card initialization succeeded!");
    // has_SD = true;
  // }
  // //esp_err_t esp_vfs_fat_register(const char* base_path, const char* fat_drive, size_t max_files, FATFS** out_fs);
  // auto r = esp_vfs_fat_register(mountPath, "/sd", maxFiles, (FATFS**) &s_SDCard);
  // printf("esp_vfs_fat_register returned %d\n", r);
  // s_SDCardMounted = (r == ESP_OK);
  // return s_SDCardMounted;
 
/*
  // One way that works with externally connected sd card slot on ttgo t8 v1.7
  sdmmc_host_t host = SDSPI_HOST_DEFAULT();
  host.slot = HSPI_HOST;
  ...
  auto r = spi_bus_initialize((spi_host_device_t)host.slot, &bus_cfg, 2);  //third/last parameter can be: SPI_DMA_DISABLED, SPI_DMA_CH1, SPI_DMA_CH2 or auto SPI_DMA_CH_AUTO.
  ...
  {  
    ...
    SPI.begin(bus_cfg.sclk_io_num, bus_cfg.miso_io_num, bus_cfg.mosi_io_num, slot_config.gpio_cs);
    printf("Attempting to reset SD card...\n");
    for(int i = 0; i < 20; i++)
    {
      printf("%d ", i);
      ...
      if(v == 0xFF)
      {
        printf("Got 0xFF back from SD card, breaking\n");
        break;
      }
    }
    printf("\n");
    ...
    //bool begin(uint8_t ssPin=SS, SPIClass &spi=SPI, uint32_t frequency=4000000, const char * mountpoint="/sd", uint8_t max_files=5, bool format_if_empty=false);
    //if (!SD.begin(slot_config.gpio_cs, SPI, 4000000, "/sd", maxFiles, formatOnFail))
    if (!SD.begin(slot_config.gpio_cs, SPI, 20000000, "/sd", maxFiles, formatOnFail))
    { 
      Serial.println("FATFS via SD.h SD card initialization failed!");
    }
    else
    ...
    //esp_err_t esp_vfs_fat_register(const char* base_path, const char* fat_drive, size_t max_files, FATFS** out_fs);
    r = esp_vfs_fat_register(mountPath, "", maxFiles, (FATFS**) &s_SDCard);
...
*/
  ...
  // one way that worked with ttgo t8 v1.7 internal sd card slot
  sdmmc_host_t host = SDSPI_HOST_DEFAULT();
  host.slot = HSPI_HOST;
  //host.max_freq_khz = 26600;  // too fast for some cards.  20000 or 19000 seems stable.  See s_SDCard->max_freq_khz
  spi_bus_config_t bus_cfg = { };
  bus_cfg.mosi_io_num      = int2gpio(MOSI);
  bus_cfg.miso_io_num      = int2gpio(MISO);
  bus_cfg.sclk_io_num      = int2gpio(CLK);
  bus_cfg.quadwp_io_num    = -1;
  bus_cfg.quadhd_io_num    = -1;
  bus_cfg.max_transfer_sz  = 4000;  //4000 works
  auto r = spi_bus_initialize((spi_host_device_t)host.slot, &bus_cfg, SPI_DMA_CH2);  //third/last parameter can be: SPI_DMA_DISABLED, SPI_DMA_CH1, SPI_DMA_CH2 or auto SPI_DMA_CH_AUTO.
  ...
  {  
    ...
    if (r == ESP_OK)
    {
      printf("sdcard max_freq_khz = %d\n", s_SDCard->max_freq_khz);
    }
