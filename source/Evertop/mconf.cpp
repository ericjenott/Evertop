...
struct ConfigDialog : public uiApp {
  ...
  void init() {
    ...
    mainFrame = new uiFrame(rootWindow(), "Machine Configuration", UIWINDOW_PARENTCENTER, Size(460, 260));
    ...
    descEdit = new uiTextEdit(mainFrame, item->desc, Point(105, y), Size(270, hh));
    ...
    FD0PathEdit = new uiTextEdit(mainFrame, item->disk[0], Point(105, y), Size(260, hh));
    //browseFD0Button = new uiButton(mainFrame, "...", Point(322, y), Size(20, hh));
    browseFD0Button = new uiButton(mainFrame, "...", Point(367, y), Size(24, hh));
    ...
    FD1PathEdit = new uiTextEdit(mainFrame, item->disk[1], Point(105, y), Size(260, hh));
    //browseFD1Button = new uiButton(mainFrame, "...", Point(322, y), Size(20, hh));
    browseFD1Button = new uiButton(mainFrame, "...", Point(367, y), Size(24, hh));
    ...
    browseHD1Button = new uiButton(mainFrame, "...", Point(322, y), Size(24, hh));
    ...
    bootDriveComboBox = new uiComboBox(mainFrame, Point(105, y), Size(80, hh), 50);
    ...
    //y = mainFrame->clientSize().height - 2;
    ...
    createImageButton = new uiButton(mainFrame, "Create Disk", Point(10, y), Size(100, hh));
  ...
  void browseFilename(uiTextEdit * edit) 
  {
    ...
    if (fileDialog("Select drive image", dir.get(), MAXVALUELENGTH, filename.get(), MAXVALUELENGTH, "OK", "Cancel", 350, 350) == uiMessageBoxResult::ButtonOK) 
    {
      if (strlen(dir.get()) > 4)
      {
  ...
  void createDiskImage() {
    ...
    if (s == 6) {
      // set hard disk size
      char str[5] = "40";
      if (ib.textInput("Hard Disk Size", "Specify Hard Disk size in Megabytes", str, 8) == InputResult::Enter)
      {
        hdSize = atoi(str);
        printf("Hard disk size entered was: %d\n", hdSize);
      }
      else
      {
        return;
      }
      if (hdSize <= 0 || hdSize > 4000) 
      {
        ib.message("Error", "Invalid Hard Disk Size. Valid range is 1 - 4000");
        return;
      }
    }
    else if (s < 0 || s > 6)
    ...
    if (s == 6) 
    {
      ...
      ib.progressBox("", "Abort", true, 380, [&](fabgl::ProgressForm * form) 
      {
        ...
        const uint32_t totSize = hdSize * (uint32_t)1048576;
        printf("totSize = %u\n", totSize);
        for (uint32_t sz = totSize; sz > 0; ) 
        {
          sz -= fwrite(buf, 1, umin(BUFSIZE, sz), file);
          //printf("sz = %u, totSize = %u, totSize - sz = %u, percentage = %d%%\n", sz, totSize, totSize - sz, (uint32_t)((uint32_t)(totSize - sz) / totSize * 100));
          if (!form->update( ( (double)(totSize - sz) / totSize * 100), "Writing %s (%u / %u bytes)", filename.get(), (totSize - sz), totSize))
          {
            break;
          }
        }
...
void loadMachineConfiguration(MachineConf * mconf)
{
  ...
  if (!fb.exists(MACHINE_CONF_FILENAME, false)) 
  {
    ...
    printf("WROTE DefaultConfFile TO SDCARD!\n");
    fclose(confFile);
  }
  // load
...
{
  ibox->setAutoOK(0);
  InputResult res = ibox->message("Please confirm", "Remove Configuration?", "No", "Yes", true);
  //if (idx > -1 && ibox->message("Please confirm", "Remove Configuration?", "No", "Yes", true) == InputResult::Enter) // InputResult::Enter = 6 // added last parameter to ibox->message to set default button to CANCEL ("No")
  //printf("res = %d\n", res);
  //printf("InputResult::Enter = %d\n", InputResult::Enter);
